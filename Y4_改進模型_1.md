...
接著，我們給每個 $\alpha$ 都過一層 [Soft-Max](https://zh.wikipedia.org/zh-tw/Softmax%E5%87%BD%E6%95%B0)，得到 $\alpha\text{'}_{1,1}, \alpha\text{'}_{1,2}, \alpha\text{'}_{1,3}, \alpha\text{'}_{1,4}$ 。這邊會得到一個機率分布那樣的結果，你不一定要用 Soft-Max，你也嘗試用 ReLU 之類的各種 Activation function。
...
\begin{cases}
v_i &= &W_va_i\\
b_1 &= &\sum\limits _i v_i\alpha\text{'}_{1,i}
\end{cases}
$$
